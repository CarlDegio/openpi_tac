...
def _is_tunnel_host(host: str | None) -> bool:
    ...
    normalized_host = host.rstrip(".").lower()
    return any(normalized_host == suffix or normalized_host.endswith(f".{suffix}") for suffix in _TUNNEL_HOST_SUFFIXES)
...
class InterfaceClient:
    ...
    def _connect(self) -> ClientConnection:
        while True:
            try:
                ...
                raise RuntimeError(
                    f"Robot websocket handshake rejected with status {exc.response.status_code}. Check the client token."
                ) from exc
    ...
    def send_config(self, config: dict[str, Any]) -> None:
        self._send_message({"type": "config", "config": config})
    ...
    def send_action(self, action: Any, obs_seq: int) -> None:
        self._send_message({"type": "action", "obs_seq": int(obs_seq), "action": action})
